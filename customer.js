...
function showProducts(){
	connection.query('SELECT * FROM products', function(err, res){
		...
		for(i=0;i<res.length;i++){
			console.log('Item ID:' + res[i].Product_Id + ' Product Name: ' + res[i].Product_Name + ' Price: ' + '$' + res[i].Price + '(Quantity left: ' + res[i].Stock_Quantity + ')')
		}
		console.log('=================================================');
		placeOrder();
		})
...
function placeOrder(){
	...
	connection.query('SELECT * FROM products WHERE id = ?', [answer.selectId], function(err, res){
		if(answer.selectQuantity > res[0].Stock_Quantity){
			...
			amountOwed = res[0].Price * answer.select_Quantity;
			...
			connection.query('UPDATE products SET ? Where ?', [{
				StockQuantity: res[0].Stock_Quantity - answer.select_Quantity
			},{
				id: answer.selectId
			}], function(err, res){});
